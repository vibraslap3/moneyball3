...
SELECT d.overallpick, d.round, d.pick, p.playername, p.Position, t.TeamName, COALESCE(sum(s.totalpoints),0) as total, COALESCE(sum(s.gamesplayed),0) as gamesplayed, COALESCE((sum(s.totalpoints)/NULLIF(COALESCE(sum(s.gamesplayed),0),0)),0)as ppg
FROM draft_data d
...
LEFT JOIN player_stats s ON d.playerid = s.playerid and d.year = s.season and s.week = tl.week

WHERE s.year = 2022 and teamname = 'Team 1'
group by d.overallpick, d.round, d.pick, p.playername, p.Position, t.TeamName
order by round, pick;



-- Shitty query to get all data by year