...
picksQuery = f"""SELECT d.overallpick, d.round, d.pick, p.playername, p.Position, t.TeamName, d.year, COALESCE(sum(s.totalpoints),0) as total, COALESCE(sum(s.gamesplayed),0) as gamesplayed, COALESCE((sum(s.totalpoints)/NULLIF(COALESCE(sum(s.gamesplayed),0),0)),0)as ppg
FROM draft_data d
LEFT JOIN player_stats s ON d.playerid = s.playerid and d.year = s.season
join players p on d.playerid = p.playerid
join Teams t on d.fantasyowner = t.teamid
WHERE d.fantasyowner = 20
group by d.overallpick, d.round, d.pick, p.playername, p.Position, t.TeamName, d.year
...
def createVisual(grid,values,teamnames,title,vmin,vmax,center):
    ...
    fig = hm.get_figure()
    fig.savefig(f".\\visualize\\visuals\\ByTeam\\{title.replace(' ','_')}.png", bbox_inches='tight')
